...
<div class="form-container px-3 pt-4">
    ...
    <form action="{{route('store.update',$store->id)}}" method="POST">
        ...
        <input type="text" placeholder="Enter Type" name="type" value="{{$store->type}}" required>

        <label for="quantity" class="form-label"><b>Date</b></label>
        <input type="number" placeholder="Enter Date" name="quantity" value="{{$store->quantity}}" required>

        <label for="price" class="form-label"><b>Cost</b></label>
        <input type="number" placeholder="Enter price" name="price" value="{{$store->price}}" required>
